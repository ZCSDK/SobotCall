...
// 转接
 311：转接中           //通话中A坐席发起咨询转接（转接功能）
 421：振铃中           //B坐席空闲被咨询转接振铃（转接功能）;
 331：转接中           //A坐席向B坐席转接，B接听后，A处于咨询转接中（转接功能） "331";
 341：转接中          //A坐席向B坐席转接，B接听后，B处于咨询转接中（转接功能） "341";

 监听
 412：振铃中          //管理坐席空闲发起监听振铃（监听强插强拆） "412";
 362：通话中          //通话中被监听|管理员振铃（监听强插强拆） "362";
 322：监听中          //管理坐席接通后发起监听（监听强插强拆） "322";
 332：通话中          //被监听坐席，通话中被监听（监听强插强拆） "332";
 342：三方通话      //管理员强插通话成功（监听强插强拆） "342";
 352：三方通话      //被监听坐席被强插（监听强插强拆） "352";
 ...
 内部通话
 425：拨号中        //空闲->A坐席发起拨号 (内部呼) "425";
 435：拨号中        //空闲->A坐席振铃(内部呼) "435";
 445：振铃中        //空闲->B坐席振铃(内部呼) "445";
 455：内部通话     //振铃->A坐席通话"455";
 465：内部通话     //振铃->B坐席通话 "465";
...
typedef NS_ENUM(NSUInteger, SobotCallAgentStatus) {
    ...
    SobotCallAgentStatus_busy = 0, // 0：忙碌
